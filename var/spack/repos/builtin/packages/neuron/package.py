...
class Neuron(Package):
    ...
    def get_neuron_archdir(self):
        ...
        """
        file_list = find(self.prefix, 'nrniv_makefile')
        # Check needed as when initially evaluated the prefix is empty.
        if file_list:
            # The file we are looking for should be under <archdir>/bin/.
    ...
    def install(self, spec, prefix):
        ...
        if spec.satisfies('+python') and not spec.satisfies('+shared'):
            raise InstallError(
                "The variant +python requires the variant +shared")
    ...
    def filter_compilers(self):
        """Remove references to spack compiler wrappers.
        ...
        nrnmakefile = join_path(
            self.prefix, neuron_archdir, 'bin/nrniv_makefile')
